document.addEventListener("DOMContentLoaded", function () {
  ...
  const closeButton = document.querySelector(".close-modal-btn"); // 모달 닫기 버튼(X 아이콘) DOM 요소 선택 // 루틴 폼 및 입력 필드
  ...
  const dayButtons = document.querySelectorAll(".day-btn"); // 월~일 요일 선택 버튼들 (NodeList) // 모달 하단 요약 정보 컨테이너 및 항목
  ...
  const summaryKcal = routineSummaryWrapper.querySelector(".summary-kcal"); // 요약: 예상 칼로리 표시 <span> // -------------------------------------------------------- // [DOM 요소 정의] - 루틴 목록 및 패널 관련 // -------------------------------------------------------- // 루틴 목록 관련 DOM 요소
  ...
  const categoryTabs = document.querySelectorAll(".category-tabs .tab-button"); // 카테고리 탭 버튼들 // 좌측 패널 (활동 요약) 관련 DOM 요소

  const activityListItems = document.querySelectorAll(
    // 활동 리스트 항목 (스포츠, 유산소 등)
    ".activity-list .activity-item"
  );
  const chartCircle = document.querySelector(".ratio-chart .chart-circle"); // 카테고리 비율을 시각화하는 차트 원형 (CSS conic-gradient 적용) // 우측 패널 DOM 요소 (목표 및 칼로리)
  ...
  let currentCaloriePerMin = 0.0; // 현재 선택된 활동 종목의 **분당** 칼로리 값 (AJAX를 통해 서버에서 가져옴) // -------------------------------------------------------- // [모달 열기/닫기 로직] // -------------------------------------------------------- // 루틴 추가 버튼 클릭 이벤트: 모달 표시 및 폼 초기화

  if (addButton && modal) {
    addButton.addEventListener("click", function () {
      modal.style.display = "flex"; // 모달을 보이게 설정 // 모달 열 때 폼과 요일 선택 초기화 (사용자 경험 개선)
  ...
  if (modal) {
    modal.addEventListener("click", function (e) {
      if (e.target === modal) {
        // 클릭된 요소가 정확히 모달 오버레이 자신일 경우에만
        modal.style.display = "none"; // 오버레이 클릭 시 모달 닫기
      }
    });
  } // -------------------------------------------------------- // [루틴 저장 및 폼 제출 로직] // -------------------------------------------------------- // 루틴 저장 폼 제출 이벤트 핸들러

  if (routineForm) {
    routineForm.addEventListener("submit", function (e) {
      e.preventDefault(); // 기본 폼 제출(페이지 이동/새로고침) 동작 방지 // 활성화된 요일 버튼에서 요일 데이터 추출 (배열 형태)

      const selectedDays = Array.from(dayButtons)
        .filter((btn) => btn.classList.contains("active")) // 'active' 클래스가 있는 버튼만 필터링
        .map((btn) => btn.getAttribute("data-day")); // 각 버튼의 'data-day' 속성값(요일)을 추출 // 폼 유효성 검증
      ...
      const estimatedKcal = Math.round(currentCaloriePerMin * durationValue); // 주당 총 예상 칼로리 계산 (1회 칼로리 * 반복 요일 수)
      const totalWeeklyKcal = estimatedKcal * selectedDays.length; // 서버 전송을 위한 데이터 객체 구성
  ...
  function saveRoutine(data) {
    fetch("/routine/create", {
      ...
      .then((result) => {
        ...
        modal.style.display = "none"; // 루틴 목록, 요약 정보, 차트 등 전체 업데이트를 위해 페이지 새로고침
  ...
  } // -------------------------------------------------------- // [이벤트 핸들러: 동적 모달 요약 정보] // -------------------------------------------------------- // 요일 버튼 클릭 이벤트: 활성화 상태 토글 및 요약 정보 업데이트
  ...
  function updateSummary() {
    ...
    summaryDayCount.textContent = `반복 요일 : ${activeDays}일`; // 2. 선택된 시간(분) 업데이트

    const durationMin = parseInt(durationMinSelect.value) || 0;
    summaryDuration.textContent = `시간 : ${durationMin}분`; // 3. 예상 칼로리 계산 및 업데이트 (1회 기준) // Math.round를 사용하여 소수점을 반올림 처리
  ...
  function updateCategoryRatio() {
    ...
    let conicGradient = []; // CSS conic-gradient 값 저장 배열 // CSS에 정의된 카테고리별 색상 (차트 시각화를 위해 미리 정의)
    ...
    for (const category in categoryCounts) {
      ...
      const angle = (percent / 100) * 360; // 360도 중 차지하는 각도 계산 // 활동 리스트 (좌측 패널) 비율 텍스트 업데이트
  ...
  function updateGoalSummary() {
    ...
    let totalExpectedKcal = 0; // 모든 루틴의 주간 총 예상 칼로리 합산 변수 // 1. 루틴 카드 순회 및 데이터 추출
    ...
    const activeDaysCount = activeDays.size; // Set의 크기 = 중복이 제거된 실제 활동 요일 수 // 2. 목표 달성률 계산 및 DOM 업데이트 // (달성 횟수 / 목표 횟수) * 100. 최대 100%를 넘지 않도록 Math.min 사용
    ...
    if (progressPercentSpan)
      progressPercentSpan.textContent = Math.round(progress) + "%"; // 진행률 텍스트 업데이트 // 3. 총 예상 칼로리 DOM 업데이트
  ...
  function filterRoutinesBy(selectedCategory) {
    ...
    allRoutineCards.forEach((card) => {
      ...
      const routineCategory = tagElement ? tagElement.textContent.trim() : ""; // 루틴 카테고리 이름 // 1. 카테고리 일치 확인: "전체"이거나 카테고리 이름이 일치해야 함

      const categoryMatch =
        selectedCategory === "전체" || routineCategory === selectedCategory; // 2. 검색어 포함 확인: 루틴 이름 또는 카테고리 이름에 검색어가 포함되어야 함

      const searchMatch =
        routineName.includes(searchTerm) ||
        routineCategory.toLowerCase().includes(searchTerm); // 두 조건이 모두 참일 때만 'grid' 레이아웃으로 표시
  ...
  function deleteRoutine(routineId) {
    fetch("/routine/delete/" + routineId, {
      ...
      headers: { "Content-Type": "application/json" }, // body: JSON.stringify({ routineId: routineId }), // ID를 URL로 전달했으므로 body는 생략 가능
    })
      .then((response) => {
        if (!response.ok) {
          throw new Error(
      ...
      .then((result) => {
        alert("루틴이 성공적으로 삭제되었습니다."); // 삭제 성공 후, 변경된 목록 반영을 위해 페이지 전체 새로고침
        window.location.reload();
      })
      .catch((error) => {
  ...
  } // -------------------------------------------------------- // [초기 실행 로직] // -------------------------------------------------------- // DOMContentLoaded 이벤트 발생 시, 페이지의 모든 동적 정보를 초기화하고 업데이트 // 1. 페이지 로드 시 초기 모달 요약 정보를 업데이트 (선택된 값 기준으로 계산)

  updateSummary(); // 2. 페이지 로드 시 좌측 패널의 루틴 카테고리 비율 정보를 계산 및 업데이트

  updateCategoryRatio(); // 3. 페이지 로드 시 우측 패널의 주간 목표 달성 횟수 및 총 예상 칼로리를 계산 및 업데이트

  updateGoalSummary(); // 4. 페이지 로드 시 '전체' 탭 기준으로 루틴 목록 초기 필터링을 적용 (숨겨진 카드가 없도록 보장)
